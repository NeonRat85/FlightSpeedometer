...
- **Lua error spam on entering combat.** The client returns player speed as a
  secret value during combat, and tainted code may not compare it, perform
  arithmetic on it, or boolean-test it. Every update tick threw
  `attempt to compare local 'yps' (a secret number value, while execution
  tainted by 'FlightSpeedometer')`. The addon now tests with `issecretvalue`
  and, while speed is withheld, holds the last good reading greyed out with
  "hidden in combat" on the sub-line, resuming automatically afterwards.
  The `isGliding` flag is guarded the same way, since it is boolean-tested.

### Added

- Harness coverage for secret values, using a stand-in whose metamethods throw
  the way a real secret does. Verified to fail if the guard is removed, so it
  reproduces the reported error rather than merely asserting the fix.
